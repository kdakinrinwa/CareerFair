...
class Googleplus {
	...
	public function __construct() {
		...
		$this->client->setClientSecret($CI->config->item('client_secret', getenv('GOOGLE_CLIENT_SECRET') ?: ''));
		$this->client->setRedirectUri($CI->config->item('redirect_uri', 'http://ola.com'));
		$this->client->setDeveloperKey($CI->config->item('api_key', getenv('GOOGLE_API_KEY') ?: ''));
